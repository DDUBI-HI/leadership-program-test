...
import { GOOGLE_FORM_URL, learningMap } from "../data/constants";
...
export default function Result({ result, onReset }) {
  ...
  const learningType = learningMap[result.learning] || learningMap.workshop;
  const scoreMax = Math.max(...Object.values(result.scores));

  return (
    <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto">
      <div className="grid lg:grid-cols-[.95fr_1.05fr] gap-6 items-stretch">
        <div className={`rounded-[2rem] bg-gradient-to-br ${meta.color} text-white p-7 md:p-9 shadow-xl overflow-hidden relative`}>
          ...
          <div className="relative">
            ...
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-4">{meta.title}</h2>
            <p className="text-lg text-white/85 leading-8 mb-6">{meta.short}</p>

            <div className="rounded-[1.75rem] overflow-hidden bg-white/10 border border-white/15 mb-5">
              <img src={meta.image} alt={meta.title} className="w-full aspect-square object-cover" />
            </div>
            ...
            <div className="rounded-3xl bg-white/10 p-5 backdrop-blur">
              <p className="text-sm text-white/70 mb-2">함께 개발하면 좋은 보조 역량</p>
        ...
        <div className="rounded-[2rem] bg-white border border-slate-200 shadow-xl p-7 md:p-9">
          <div className="mb-7">
            <h3 className="text-2xl font-bold text-slate-950 mb-3">추천 이유</h3>
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-8">
            ...
            <div className="rounded-3xl bg-slate-50 p-5 border border-slate-100">
              ...
              <p className="font-bold text-slate-900">진단 → 실습 → 액션플랜</p>
            </div>
          </div>

          <div>
            <h3 className="text-2xl font-bold text-slate-950 mb-4">추천 프로그램</h3>
      ...
      <div className="grid lg:grid-cols-[1fr_.82fr] gap-6 mt-6">
        <div className="rounded-[2rem] bg-white border border-slate-200 shadow-sm p-7">
          <h3 className="text-xl font-bold text-slate-950 mb-5">역량별 응답 분포</h3>
          <div className="space-y-4">
            {Object.entries(result.scores).map(([tag, score]) => {
              const label = typeMeta[tag].title;
              const width = scoreMax === 0 ? 0 : Math.max(10, Math.round((score / scoreMax) * 100));
              return (
                <div key={tag}>
        ...
        <div className="rounded-[2rem] bg-slate-950 text-white p-7 shadow-xl">
          <h3 className="text-2xl font-bold mb-3">우리 조직에 맞게 설계하고 싶다면?</h3>
          <p className="text-slate-300 leading-7 mb-6">
            추천 결과를 바탕으로 대상자, 교육 시간, 핵심 이슈에 맞춘 맞춤형 리더십 학습 여정을 제안드릴 수 있습니다.
          </p>

          <div className="space-y-3 mb-7 text-sm text-slate-300">
            <div className="flex gap-2"><Icon name="check" className="w-5 h-5 text-white" /> 계층별 과정 설계</div>
            <div className="flex gap-2"><Icon name="check" className="w-5 h-5 text-white" /> 테마별 모듈 조합</div>
            <div className="flex gap-2"><Icon name="check" className="w-5 h-5 text-white" /> 특강·워크숍·코칭형 운영 가능</div>
          </div>
